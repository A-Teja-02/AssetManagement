...
const Repairs = () => {
  ...
  const handleAddSubmit = (e) => {
    ...
    alert("New repair request created successfully!");
  };
  ...
  const handleUpdateSubmit = (e) => {
    ...
    alert(`Successfully added status update to ${selectedRepair.id}!`);
  };

  const handleCancelRequest = () => {
    if (!selectedRepair) return;
    if (confirm(`Cancel repair request ${selectedRepair.id}?`)) {
      addRepairUpdate(selectedRepair.id, "Cancelled", "Repair request cancelled by administrator.");
      alert(`Cancelled repair request ${selectedRepair.id}`);
    }
  };
